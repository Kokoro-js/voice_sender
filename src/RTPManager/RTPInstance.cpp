...
uvgrtp::media_stream* RTPInstance::createStream(const std::string& stream_id, const ChannelJoinedData& streamInfo, RTP_FORMAT format, int flags) {
    if (streamInfo.rtcp_mux) flags |= RCE_RTCP_MUX;

    auto stream_ = session->create_stream(streamInfo.port, format, flags);
    if (!stream_) {
        PLOGE << "Failed to create stream for ID: " << stream_id << " IP " << remote_address << " Port " << streamInfo.port;
        return nullptr;
    }
